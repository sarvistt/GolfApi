...
async def getCanoeClubData():
    async with async_playwright() as p:
        ...
        await page.select_option('select[name="Date"]', xy)
        await page.select_option('select[name="SearchTime"]', xf)
        await page.evaluate(f"""
    () => {{
        const radio = document.querySelector('input[type="radio"][name="Holes"][value="{xxx}"]');
        if (radio) {{
            radio.checked = true;
...
""")
        ...
        await page.evaluate(f"""
    () => {{
        const radio = document.querySelector('input[type="radio"][name="Players"][value="{xx}"]');
        if (radio) {{
            radio.checked = true;
            radio.dispatchEvent(new Event('change', {{ bubbles: true }}));
        }}
